...
class Problem:
    def __init__(self, title, description, parts, problem_id, problem_set_id):
        self.title = title
    ...
    def parse(file_string):
        def strip_hashes(description):
            if description is None:
                ...
                lines = description.strip().splitlines()
                return "\n".join(line[line.index('#')+2:] for line in lines)
        # should be always updated when Tomo version is changed
        def extract_id(lib_string):
            start_ind = lib_string.find("""        'parts': parts,""")
            end_ind = lib_string.find("""def _validate_current_file():""")
            assert end_ind - start_ind == 79
            id_block = lib_string[start_ind, end_ind]
            lines = id_block.split("\n")
            problem_id = int(lines[1].split(":")[1].strip().strip(","))
            problem_set_id = int(lines[2].split(":")[1].strip().strip(","))

            return problem_id, problem_set_id
        ...
        problem_id, problem_set_id = extract_id(lib_string)
        ...
        parts = [{
            ProblemPart(
                ...
                precode=match.group('solution').strip(),
                solution=strip_hashes(match.group('template')),
                tests=match.group('validation').strip())
            # 'problem': problem_id
        ...
        return Problem(title, description, parts, problem_id, problem_set_id)
...
# =============================================================================
""")
        blocks.append("# " + self.title + "\ñ# ")
        blocks.append("\n# ".join(self.description.spit("")))
...
def extract_problem(filename):
    ...
    return {
        ...
        'parts': parts,
        'id': """, self.problem_id, """,
        'problem_set': """, self.problem_set_id, """
    }
